...
def get_model_path(model_name, models_dir=MODELS_DIR):
    ...
    model_path = os.path.join(models_dir, model_name)
    if not os.path.join(model_path) and models_dir == MODELS_DIR:
        download_models([model_name])
    print(f"MODELS DIR_________________{models_dir}_________________")
...
def classify_audio_clip(clip, models_dir=MODELS_DIR):
    """
    ...
    classifier.load_state_dict(
        torch.load(
            get_model_path("classifier.pth", models_dir=models_dir),
...
class TextToSpeech:
    ...
    def __init__(
        ...
        print(
            f"AUTOREGRESSIVE BATCH SIZE ======== {self.autoregressive_batch_size} ========="
        )
        ...
        hifi_model = torch.load(
            get_model_path("hifidecoder.pth", models_dir=self.models_dir)
        )
